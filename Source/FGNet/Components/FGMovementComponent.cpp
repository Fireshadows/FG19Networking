...
void UFGMovementComponent::Move(FFGFrameMovement& FrameMovement)
{
	...
	SlideAlongSurface(Delta, 10.0f - Hit.Time, Hit.Normal, Hit);
...
void UFGMovementComponent::SetFacingDirection(const FVector& InFacingRotation, float InRotationSpeed)
{
	Internal_SetFacingRotation(InFacingRotation.Rotation(), InRotationSpeed);
}
